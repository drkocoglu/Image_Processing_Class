...
%     imOrig = imread(new_path);
    ...
    for k = 1:N
        ...
        imnew = imOrig;
        imnew(~card_detected) = 0;
        %imnew = adapthisteq(imnew);
...
% %        binarize and smooths the image
%         imBin = double(imbinarize(imnew));
%         imSmooth = imgaussfilt(imBin, 10);
%         
%         
% %        uses the gradient direction https://www.mathworks.com/help/images/ref/label2rgb.html
%to find the angle
...
%         close HIST
%         %rotates the image
%         imRot = imrotate(imnew, rot_ang, 'crop');
        ...
        if middle_half <= 13
            ...
            if middle <= 63
                predictedSuit = 'Hearts';fig.WindowState = 'fullscreen';
            else
                predictedSuit = 'Clubs';
            end
        end
        
        mask{1} = fliplr(boundaries{k});
        L = poly2label(mask,k,size(imOrig));
        
        subplot(2,N,k)
        %imshow(imnew)
        imshow(labeloverlay(imOrig,L));
        subplot(2,N,k)
        
        %To show the suit and rank
        axis off
        text(700, 1800, ['Suit: ' predictedSuit 'Rank: ' predictedRank ], ...
...
%     roiPositions = cell(N,1);
%     roilabelID = 1:N;
%     for id = 1:N
%         roiPositions{id} = fliplr(boundaries{id});
%     end
%     
%     L = poly2label(roiPositions,roilabelID,size(imOrig));
%     figure;
%    imshow(labeloverlay(imOrig,L,'IncludedLabels',roilabelID));
