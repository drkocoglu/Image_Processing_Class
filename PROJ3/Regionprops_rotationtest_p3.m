...
%     clear mytext;
    ...
    complementimage = imcomplement(binarizedimage);
    % Process the image
    ...
    %% Rotate images
    %Pixels_inside_crd = bwlabel(binarizedimage);
    %disp(Pixels_inside_crd);
    %binarizedimage(Pixels_inside_crd>0) = 1;
    binarizedimage = imfill(binarizedimage,'holes');
    Crd = regionprops(binarizedimage,'Area','Orientation','Extrema');
    disp(Crd.Extrema);
    max_x = max(Crd.Extrema(:,1));
    min_x = min(Crd.Extrema(:,1));
    max_y = max(Crd.Extrema(:,2));
    min_y = min(Crd.Extrema(:,2));
    rotation_angle = 90-Crd.Orientation;
    imRot = imrotate(currentimage, rotation_angle, 'crop');
...
%     close HIST
    ...
    for ii = 1:size(Crd.Extrema,1)
        drawpoint('Position',Crd.Extrema(ii,:));
    end

%     drawpoint('Position',[min_y,max_x]);
%     drawpoint('Position',[max_y,min_x]);
%     drawpoint('Position',[max_y,min_x]);
    subplot(1,2,2) 
    imshow(imRot);
    %%
